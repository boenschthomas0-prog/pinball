...
' ===========================================================================
' Pricing-HINWEIS (2026-05-27): Der ursprüngliche Pricing-Block ("coin1..4",
' "unit", "freeplay", "pricing" usw.) war wirkungslos. VPinMAME COM erlaubt
' über IGameSettings nur eine fixe Whitelist von ~37 Keys (cheat, sound,
' samples, dmd_*, samplerate, cabinet_mode, ignore_rom_crc, showpindmd, ...).
'   Quelle: vpinball/pinmame, src/win32com/ControllerGameSettings.cpp
'   (Get/SetControlValues, alle CComBSTR-Keys). Keine Pricing-Keys vorhanden.
'
' AFM-Pricing (1 Coin = 1 Credit, kein Replay-/Match-Award) muss daher
' entweder im ROM-Service-Menü (Coin-Door auf + Enter Adjustments A.1.05,
' A.1.16, A.1.19, Pricing A.2.xx) oder durch Vorab-Schreiben des NVRAM
' (~/.vpinball/pinmame/nvram/afm_113b.nv inkl. korrekter WPC-Checksums)
' gesetzt werden.
'   NVRAM-Adjustment-Offsets:
'     7090  A.1.14 Replay         (1=on, 0=off)  -> 0 = kein Replay-Knocker
'     7095  A.1.16 Match Award    (1=on, 0=off)  -> 0 = kein "MATCH 00"
'     7099  A.1.19 Match Feature  (0=off, 7=10%) -> 0 = aus
'   Quelle: tomlogic/pinmame-nvram-maps maps/williams/wpc/afm_113.map.json
'
' Hier bewusst KEIN Settings.Value(...)-Schreibversuch mehr — würde nur
' fälschlich Aktivität suggerieren. Stattdessen: NVRAM-Snapshot pflegen.
' ===========================================================================
.HandleKeyboard = 0
.ShowTitle = 0
.ShowDMDOnly = 1
...
' === TIMER-TICK-FIX (2026-05-27): siehe statische Sub PinMAMETimer_Timer am
' Datei-Ende (kann hier nicht stehen, weil dies AFM_Init ist; VBScript verbietet
' nested Sub-Definitionen).
...
Set bsTrough = New cvpmTrough
    With bsTrough
        ...
        ' === DRAIN-REFEED-FIX (2026-05-27): InitEntrySounds setzt die "add"/"entry"/"entryBall"-Keys
        ' im cvpmDictionary leer vor, damit AddBall() unter Wine-VBS nicht mehr in den
        ' dictionary_Invoke Type-Mismatch (hres=-2146795477) auf mSounds.Item("add") rennt.
        .InitEntrySounds "", "", ""
        .Balls = 4
    End With

' === TROUGH-HARDENING (2026-05-27): Belt-and-Suspenders nach bsTrough.Balls=4.
' Wine-VBS kann cvpmTrough.UpdateTroughSwitches gelegentlich nicht durchziehen
' (mSlot-State korrekt, aber Controller.Switch(...)-Calls verschluckt). Wir setzen
' die Trough-Switches und den Plunger-EOS-Switch hier explizit, damit die ROM bei
' Cold-Boot zuverlaessig "4 Baelle im Trough, Plunger frei" sieht.
On Error Resume Next
Controller.Switch(32) = 1   ' Trough 1 (exit slot)
Controller.Switch(33) = 1   ' Trough 2
Controller.Switch(34) = 1   ' Trough 3
Controller.Switch(35) = 1   ' Trough 4 (entry slot)
Controller.Switch(11) = 0   ' Plunger EOS frei
On Error Goto 0
...
' === DRAIN-REFEED-FIX (2026-05-27): AFM-ROM braucht einen Puls auf sw31 (Outhole),
' damit der Drain registriert und ein neuer Ball ausgegeben wird. cvpmTrough.AddBall
' alleine reicht nicht, weil EntrySw=0 ist und die ROM keine sw32-35-Transition
' alleine als Drain-Event akzeptiert (sw31 ist der Trigger). On Error Resume Next
' schützt vor Wine-VBS dictionary_Invoke beim mSounds.Item("add") Default-Property.
' === DRAIN-DIRECT (2026-05-27): cvpmTrough.AddBall + vpmTimer.PulseSw 31
' nutzen die vpmTimer-Queue (mNeedUpdate / NeedUpdate). Unter Wine-VBS
' tickt die nicht zuverlaessig -> Ball wird zerstoert, aber bsTrough.Balls
' und Trough-Switches werden nicht aktualisiert -> ROM sieht "Ball weg"
' und feuert keine SolRelease. Direct-Pfad: Counter selber inkrementieren,
' Property Let triggert UpdateTroughSwitches synchron.
Sub Drain_Hit
    On Error Resume Next
    ...
    debug.print "DBG: Drain direct, bsTrough.Balls now=" & bsTrough.Balls
    On Error Goto 0
End Sub
Sub Drain1_Hit
    On Error Resume Next
...
Sub AFM_KeyDown(ByVal Keycode)
	' === DEBUG: jeder KeyDown loggen (diagnose Flipper-Tasten)
	debug.print "DBG: AFM_KeyDown keycode=" & keycode & " LF=" & LeftFlipperKey & " RF=" & RightFlipperKey

	' === UX-FIX (2026-05-27): End-Taste (keyCoinDoor=207) komplett blockieren.
	If keycode = keyCoinDoor Then Exit Sub
	...
	If keycode = keyInsertCoin1 or keycode = keyInsertCoin2 or keycode = keyInsertCoin3 or keycode = keyInsertCoin4 Then
			...
			End Select
			' === COIN-FIX v3 (2026-05-27): Switch DIREKT synchron setzen, statt
			' via vpmTimer.PulseSw -> Queue (Queue wird unter Wine-VBS nicht
			' rechtzeitig abgearbeitet, Switch bleibt False, ROM sieht keinen Coin).
			' KeyUp setzt den Switch zurueck, simuliert einen kurzen Coin-Puls.
			On Error Resume Next
			If keycode = keyInsertCoin1 Then Controller.Switch(1) = True
			If keycode = keyInsertCoin2 Then Controller.Switch(2) = True
			If keycode = keyInsertCoin3 Then Controller.Switch(3) = True
			If keycode = keyInsertCoin4 Then Controller.Switch(4) = True
			debug.print "DBG: Switch nach direct-set, keycode=" & keycode & _
			            " sw1=" & Controller.Switch(1) & _
			            " sw2=" & Controller.Switch(2) & _
			            " sw3=" & Controller.Switch(3) & _
			            " sw4=" & Controller.Switch(4)
			On Error Goto 0
			Exit Sub
	End If
	...
	If keycode = StartGameKey Then
		soundStartButton()
		' === START-FIX (2026-05-27): Direct-Set wie Coin. swStart=13 laut PinMAME afm.c.
		On Error Resume Next
		Controller.Switch(13) = True
		debug.print "DBG: Start direct-set sw13=" & Controller.Switch(13)
		On Error Goto 0
	End If

	' === FLIPPER-DIRECT (2026-05-27): vpmKeyDown (WPC.vbs Z106-110) macht die Rotation
	' via vpmFlips.FlipL/R, plus .Switch(swLLFlip) = True. Wenn vpmKeyDown irgendwo
	' frueher in einem Type-Mismatch haengt, kommt der Flipper-Case nie an. Wir
	' setzen Switch und Rotation hier direkt; vpmKeyDown danach ist Doppel-Set (OK).
	' === FLIPPER-FIX v2 (2026-05-27): SolLFlipper/SolRFlipper rufen LF.Fire/RF.Fire
	' (nFozzy FlipperPolarity-Klasse) - das macht die echte Animation/Physik.
	' Wenn der Sol-Callback unter Wine nicht zuverlaessig durchkommt, fehlt die
	' Animation. Daher LF.Fire direkt im KeyDown rufen.
	If keycode = LeftFlipperKey Then
		On Error Resume Next
		Controller.Switch(114) = True
		LF.Fire
		On Error Goto 0
	End If
...
Sub AFM_KeyUp(ByVal Keycode)
	If keycode = keyCoinDoor Then Exit Sub   ' UX-FIX: End-Taste auch hier blockieren

	' Coin-Switches synchron zurueck setzen (Gegenstueck zu KeyDown direct-set)
	On Error Resume Next
	If keycode = keyInsertCoin1 Then Controller.Switch(1) = False
	If keycode = keyInsertCoin2 Then Controller.Switch(2) = False
	If keycode = keyInsertCoin3 Then Controller.Switch(3) = False
	If keycode = keyInsertCoin4 Then Controller.Switch(4) = False
	If keycode = StartGameKey  Then Controller.Switch(13) = False
	On Error Goto 0
...
' Linux-Workaround: VPX-Standalone bindet das Element 'AFM' offenbar nicht
' an AFM_KeyDown/AFM_KeyUp. Aliase fuer haeufige Element-Namen ergaenzen.
Sub Table1_KeyDown(ByVal keycode) : AFM_KeyDown keycode : End Sub
Sub Table1_KeyUp(ByVal keycode)   : AFM_KeyUp keycode   : End Sub
Sub table1_KeyDown(ByVal keycode) : AFM_KeyDown keycode : End Sub
Sub table1_KeyUp(ByVal keycode)   : AFM_KeyUp keycode   : End Sub
...
Sub SolRelease(Enabled)
	If Enabled And bsTrough.Balls > 0 Then
		' === BALL-LAUNCH-FIX v2 (2026-05-27): Direct-Pfad + Trough-Counter dekrementieren.
		' Property Let bsTrough.Balls ruft intern UpdateTroughSwitches auf - das setzt
		' Controller.Switch(32..35) entsprechend der Restanzahl. Damit "sieht" ROM
		' den Trough sich leeren und feuert SolRelease NICHT noch einmal.
		On Error Resume Next
		Controller.Switch(31) = True
		vpmCreateBall(BallRelease).Kick 90, 4
		bsTrough.Balls = bsTrough.Balls - 1   ' triggert UpdateTroughSwitches -> sw32..35
		Controller.Switch(31) = False
		debug.print "DBG: SolRelease direct-kick, bsTrough.Balls now=" & bsTrough.Balls
		On Error Goto 0
	End If
End Sub
...
' Wine-VBScript-Safety: PolarityCorrect crashed mit Type-Mismatch wenn activeball
' Empty oder ungueltig ist (Dictionary-Invoke dispId=7). On Error wrap.
Sub TriggerLF_Hit()
    On Error Resume Next : LF.Addball activeball : On Error Goto 0
End Sub
Sub TriggerLF_UnHit()
    On Error Resume Next : LF.PolarityCorrect activeball : On Error Goto 0
End Sub
Sub TriggerRF_Hit()
    On Error Resume Next : RF.Addball activeball : On Error Goto 0
End Sub
Sub TriggerRF_UnHit()
    On Error Resume Next : RF.PolarityCorrect activeball : On Error Goto 0
End Sub
...
Function LinearEnvelope(xInput, xKeyFrame, yLvl)
	dim y 'Y output
	dim L 'Line
	' Wine-VBScript-Safety: xInput kann Empty werden wenn cor.ballvel-Array zu
	' klein ist (neuer Ball mit hoher ID). Dann terminiert die for-Schleife ohne
	' L zu setzen, und xKeyFrame(L-1) wirft Type-Mismatch (Index -1). Bail-Out.
	If IsEmpty(xInput) Or Not IsNumeric(xInput) Then LinearEnvelope = 1 : Exit Function
	If uBound(xKeyFrame) < 1 Then LinearEnvelope = yLvl(0) : Exit Function
	dim ii : for ii = 1 to uBound(xKeyFrame)        'find active line
		if xInput <= xKeyFrame(ii) then L = ii : exit for : end if
	Next
	if xInput > xKeyFrame(uBound(xKeyFrame) ) then L = uBound(xKeyFrame)        'catch line overrun
	If IsEmpty(L) Then L = 1                                                    'safety: L noch immer ungesetzt
	If L < 1 Then L = 1                                                         'safety: L-1 darf nicht negativ werden
...
'
'//////////////////////////////////////////////////////////////////////
' === TIMER-TICK-FIX (2026-05-27): Statische PinMAMETimer_Timer-Sub.
' Normalerweise erzeugt core.vbs::vpmBuildEvent (~Z2537) diese Sub dynamisch via
' ExecuteGlobal "Sub PinMAMETimer_Timer : vpmTimer.Update : End Sub". Unter
' Wine-VBScript schlaegt das gelegentlich fehl -> Timer feuert ins Leere,
' vpmTimer.Update wird NIE aufgerufen -> die gesamte PulseSw/AddTimer-Queue
' bleibt liegen. Folge: Coin-Switches (PulseSw 1..4), Drain-Outhole (PulseSw 31),
' Sol-Pulses kommen nie beim ROM an.
' Statische Sub erzwingt den Tick. Muss im globalen Scope stehen (nicht in
' einer anderen Sub) - VBScript verbietet nested Sub-Definitionen.
'//////////////////////////////////////////////////////////////////////
Dim gDbgTickCount
gDbgTickCount = 0
Sub PinMameTimer_Timer
    gDbgTickCount = gDbgTickCount + 1
    If gDbgTickCount = 1 Then
        On Error Resume Next
        PlaySound "fx_Knocker", 0, 1, 0, 0
        On Error Goto 0
    End If
    vpmTimer.Update
End Sub

' === FLIPPER-VISUAL-FIX (2026-05-27): LeftFlipper/RightFlipper sind is_visible=false.
' Die visuelle Repraesentation sind die Primitives batleft/batright (+ Schatten).
' Diese muessen pro Frame mit LeftFlipper.CurrentAngle synchronisiert werden.
' Die Sub LeftFlipperTimer_Timer war wohl im embedded .vpx-Skript, ist im Sidecar
' aber weg - daher hier statisch nachbauen. RotZ-Offset: batleft startet bei 0,
' LeftFlipper.StartAngle=119.3, EndAngle=70.4 -> Diff -48.9 wenn flipper hoch.
Sub LeftFlipperTimer_Timer
    On Error Resume Next
    batleft.RotZ       = LeftFlipper.CurrentAngle - LeftFlipper.StartAngle
    batleftshadow.RotZ = LeftFlipper.CurrentAngle - LeftFlipper.StartAngle
    On Error Goto 0
End Sub

Sub RightFlipperTimer_Timer
    On Error Resume Next
    batright.RotZ       = RightFlipper.CurrentAngle - RightFlipper.StartAngle
    batrightshadow.RotZ = RightFlipper.CurrentAngle - RightFlipper.StartAngle
    On Error Goto 0
End Sub
